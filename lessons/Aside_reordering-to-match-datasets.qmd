---
title: "Using the match() Function"
description: |
  This lesson introduces participants to the use of the `match()` function for aligning and reordering data. Participants learn how to manually reorder vectors, how `match()` simplifies the process for larger datasets and how to apply it. Through hands‑on examples, participants practice generating index vectors, handling missing matches and reordering both vectors and data frames to ensure consistent ordering.
author:
  - Mary Piper
  - Meeta Mistry
  - Radhika Khetani
...
```{r}
#| label: create_teaching_team_vector
...
```{r}
#| label: return_specific_elements
...
```{r}
#| label: extract_reorder_elements
...
::: {.callout-tip}
# [**Exercise 1**](Aside_reordering-to-match-datasets-Answer_key.qmd#exercise-1)
...
```{r}
#| label: create_index_from_match_first_second
...
```{r}
#| label: remove_values
# Create new first and second vector
first <- c("A","B","C","D","E")
...
```{r}
#| label: match_first_second_missing_values
# Run match on new first and second vectors
match(first,second)
```
...
```{r}
#| label: reorder_second_missing_values
# Rearrange second using match
...
```{r}
#| label: create_metadata_match_index
# Create match index vector
genomic_idx <- match(rownames(metadata), colnames(rpkm_data))

# Print match index vector
genomic_idx
```
...
```{r}
#| label: rendered_reordered rpkm
#| echo: false
# Load DT package
library(DT)

# Render the reordered counts
datatable(rpkm_ordered)
```
...
```{r}
#| label: all_rownames_metadata_colnames_rpkm_ordered
...
::: {.callout-tip}
# [**Exercise 2**](Aside_reordering-to-match-datasets-Answer_key.qmd#exercise-2)
