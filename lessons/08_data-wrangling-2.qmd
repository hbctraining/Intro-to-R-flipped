...
description: |
  This lesson introduces participants to more data wrangling skills in R using dataframes, matrices and lists. Participants learn how to subset, merge and wrangle data structures using indices, names and logical expressions. By practicing different extraction methods and working with multi‑component lists, participants gain confidence in navigating and reshaping data for analysis.
author:
  - Mary Piper
  - Meeta Mistry
  - Radhika Khetani
...
1. To extract it, just like with vectors, we give the name of the data frame that we want to extract from, followed by the square brackets (`metadata[ ]`). 
2. Now inside the square brackets we **give the coordinates** or indices for the rows in which the value(s) are present, followed by a comma, then the coordinates or indices for the columns in which the value(s) are present (`metadata[rows, columns]`). 
...
```{r}
#| label: subset_metadata_third_column_drop_false
...
::: {.callout-tip}
# [**Exercise 1**](08_data-wrangling-2-Answer_key.qmd#exercise-1)
...
::: {.callout-tip}
# [**Exercise 2**](08_data-wrangling-2-Answer_key.qmd#exercise-2)
...
```{r}
#| label: second_object_list_class
...
```{r}
#| label: first_object_list_first_element
...
::: {.callout-tip}
# [**Exercise 3**](08_data-wrangling-2-Answer_key.qmd#exercise-3)
...
```{r}
#| label: subset_list_by_list_names
# Extract "df" component
list1$df
```

::: {.callout-tip}
# [**Exercise 4**](08_data-wrangling-2-Answer_key.qmd#exercise-4)
