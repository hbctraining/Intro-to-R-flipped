...
description: |
  This lesson introduces participants to plotting and data visualization using the `ggplot2` package. Participants learn the syntax of graphics, how to choose appropriate geoms for different data types, how to customize plot aesthetics and how to export figures. The lesson builds figures using layered graphics, meaningful color palettes and consistent themes.
author:
  - Meeta Mistry
  - Mary Piper
  - Radhika Khetani
...
```{r}
#| label: just_ggplot_and_geom_point_functions
#| eval: false
...
::: {.callout-note}
RStudio provides this very [useful cheatsheet](https://rstudio.github.io/cheatsheets/html/data-visualization.html) for plotting using `ggplot2`. Different example plots are provided and the associated code (i.e which `geom` or `theme` to use in the appropriate situation.)
:::

::: {.callout-tip}
# [**Exercise 1**](Aside_ggplot2-Answer_key.qmd#exercise-1)
...
```{r}
#| label: load_rcolorbrewer_view_palettes
...
::: {.callout-tip}
# [**Exercise 2**](Aside_ggplot2-Answer_key.qmd#exercise-2)
...
```{r}
#| label: add_color_geom_col
# Add color to geom_col plot
ggplot(bp_plot) +
  geom_col(aes(x = GO_term, y = overlap.size),
           fill = "royalblue",
           color = "black")
...
```{r}
#| label: add_theme_title_axes_geom_col
# Add our themes, a title and axes labels
ggplot(bp_plot) +
  geom_col(aes(x = GO_term, y = overlap.size),
           fill = "royalblue",
           color = "black") +
...
Now we might be fairly happy with our plot, but the x-axis labelling needs some help. Within the `theme()` layer, we can change the orientation of the x-axis labels with the `angle` argument and align the labels to the x-axis with the `hjust` argument.

```{r}
#| label: rotate_x_axis_labels
...
```{r}
#| label: increase_margin
# Increase the margin size to fit the x-axis better
ggplot(bp_plot) +
  geom_col(aes(x = GO_term, y = overlap.size),
           fill = "royalblue",
           color = "black") +
...
```{r}
#| label: create_function_example
#| eval: false
# DO NOT RUN
# Example syntax for a function
...
::: {.callout-tip}
# [**Exercise 3**](Aside_ggplot2-Answer_key.qmd#exercise-3)
